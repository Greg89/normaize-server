...
/// <summary>
/// Handler for GetDataSummaryQuery
/// </summary>
public class GetDataSummaryQueryHandler : IRequestHandler<GetDataSummaryQuery, DataSummaryDto>
{
    private readonly IDataSetRepository _dataSetRepository;
    private readonly IDataSummaryService _dataSummaryService;
    private readonly ILogger<GetDataSummaryQueryHandler> _logger;

    public GetDataSummaryQueryHandler(
        IDataSetRepository dataSetRepository,
        IDataSummaryService dataSummaryService,
        ILogger<GetDataSummaryQueryHandler> logger)
    {
    ...
    public async Task<DataSummaryDto> Handle(GetDataSummaryQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Attempted to get data summary for deleted dataset {DataSetId}", request.DataSetId);
        ...
        var summary = _dataSummaryService.GenerateDataSummary(dataSet, data);
        summary.ProcessingTime = stopwatch.Elapsed;
        summary.GeneratedAt = DateTime.UtcNow;

        _logger.LogInformation("Successfully generated data summary for dataset {DataSetId} in {ElapsedMs}ms",
            request.DataSetId, stopwatch.ElapsedMilliseconds);

        return summary;
    }

    private void ValidateInputs(GetDataSummaryQuery request)
    {
